...
for IDRAC_IP in 10.19.143.24 10.19.143.23 10.19.143.22 10.19.143.20 10.19.143.18 10.19.143.17
do
	echo "Attaching iso to BMH ${IDRAC_IP}"
	${CONTAINER_TOOL} run --network host --rm -it quay.io/alosadag/racadm:latest -r $IDRAC_IP -u root -p 100Mgmt- remoteimage -d
	${CONTAINER_TOOL} run --network host --rm -it quay.io/alosadag/racadm:latest -r $IDRAC_IP -u root -p 100Mgmt- remoteimage -c -l http://10.19.142.248:8080/discovery_image_${CLUSTER_NAME}.iso
	${CONTAINER_TOOL} run --network host --rm -it quay.io/alosadag/racadm:latest -r $IDRAC_IP -u root -p 100Mgmt- set iDRAC.VirtualMedia.BootOnce 1
	${CONTAINER_TOOL} run --network host --rm -it quay.io/alosadag/racadm:latest -r $IDRAC_IP -u root -p 100Mgmt- set iDRAC.ServerBoot.FirstBootDevice VCD-DVD
	${CONTAINER_TOOL} run --network host --rm -it quay.io/alosadag/racadm:latest -r $IDRAC_IP -u root -p 100Mgmt- serveraction powercycle
	sleep 60
done
...
aicli update cluster -P api_vip=${API_VIP} $CLUSTER_NAME
